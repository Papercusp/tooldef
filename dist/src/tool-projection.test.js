...
import { z } from 'zod';
import { registerProjectedTool, unregisterProjectedToolsForPlugin, lookupByMcpName, resolveMcpName, normalizeMcpName, lookupByHttpPath, listAllProjectedTools, listMcpProjections, PROJECTED_TOOL_REGISTRY_SOURCE, projectedToolRegistryRevision, projectedToolCallContract, assertProjectedToolCallContract, renderProjectedToolCall, projectedToolCorrectiveCalls, ProjectedToolContractError, classifyEventWire, ToolRegistrationError, _resetProjectionRegistryForTests, } from './tool-projection';
const noop = async () => ({
...
describe('resolveMcpName (WI-3930 — tolerant tool-name resolution)', () => {
    ...
    const register = () => registerProjectedTool(baseTool({ expose: { mcp: { name: 'curation:state-of-pot' } } }));
...
describe('registerProjectedTool', () => {
    ...
    it('chunk-reservation error message points at largeOutput', async () => {
        ...
        expect(() => registerProjectedTool(baseTool({
            ...
            events: { chunk: z.object({ ref: z.string() }) },
        }))).toThrow(/largeOutput/);
    });
...
describe('listMcpProjections', () => {
    ...
    it('surfaces events schemas as JSON-Schema when the tool declares them', () => {
        ...
        expect(list[0].events.delta.$schema).toBeUndefined();
    });
...
describe('registry-derived executable tool contracts (P-002)', () => {
    ...
    it('fails structured corrective-call conformance on missing tools, required keys, enums, and undeclared keys', () => {
        ...
        const source = (name, tool, args) => registerProjectedTool(baseTool({
            expose: { mcp: { name } },
...
describe('emitToSseSink', () => {
    ...
    function recSink() {
        ...
        return {
            events,
            sink: {
                event(name, value) { events.push({ kind: 'event', name, data: value }); },
                eventRaw(name, value) { events.push({ kind: 'raw', name, data: value }); },
            },
        };
    }
    it('z.string() events go through eventRaw with the raw text', async () => {
        ...
        emitToSseSink(sink, { eventWireKinds: { delta: 'string' } }, 'delta', 'hello world');
    ...
    it('object events go through event with the object passed through', async () => {
        const { emitToSseSink } = await import('./tool-projection');
        const { sink, events } = recSink();
        emitToSseSink(sink, { eventWireKinds: { cost: 'json' } }, 'cost', { usd: 0.5 });
    ...
    it('binary events emit the self-describing envelope (cross-transport unification)', async () => {
        ...
        emitToSseSink(sink, { eventWireKinds: { chunk: 'binary' } }, 'chunk', bytes);
    ...
    it('binary kind without Uint8Array data falls back to JSON (defensive)', async () => {
        ...
        emitToSseSink(sink, { eventWireKinds: { chunk: 'binary' } }, 'chunk', { wrong: 'shape' });
