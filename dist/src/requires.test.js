...
const makeTool = (requires, over = {}) => ({
    pluginName: 'fixture',
    description: 'fixture',
    inputSchema: { type: 'object' },
    capabilities: [],
    expose: { mcp: { name: 'fix.tool' } },
    fn: vi.fn(async () => ({ content: [{ type: 'text', text: 'ok' }] })),
    requires,
    ...over,
});
...
describe('preconditions step — reject path', () => {
    ...
    it('audits the denial with gate "precondition"', async () => {
        ...
        expect(auditAuth).toHaveBeenCalledWith(expect.objectContaining({ gate: 'precondition', decision: 'deny', tool: 'fix.tool' }));
    });
    it('evaluates over ctx too — `any` combinator over args + ctx', async () => {
        ...
        const neither = await runDispatchStack(makeTool([spec]), 'fix.tool', {}, MAKE_CTX({ harnessSlug: undefined }), {});
    ...
    it('evaluates resolved state and fails closed when the resolver throws', async () => {
        const ok = await runDispatchStack(makeTool([{ when: { 'state.ready': true }, state: async () => ({ ready: true }) }]), 'fix.tool', {}, MAKE_CTX(), {});
        expect(ok.ok).toBe(true);
        const thrown = await runDispatchStack(makeTool([
            {
                id: 'st',
...
describe('preconditions step — auto-correct path ({ fire, then: "retry" })', () => {
    it('fires the corrective tool, re-resolves state, and proceeds when the retry holds', async () => {
        ...
        expect(deps.auditAuth).toHaveBeenCalledWith(expect.objectContaining({
            gate: 'precondition',
            decision: 'allow',
