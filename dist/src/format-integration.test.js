...
function defineListTool(name, opts = {}) {
    defineTool({
        name,
...
describe('raw-ToolResult re-encode on the MCP transport (definetool-token-optimization-adoption P-002)', () => {
    ...
    function definePrincipalRaw(name, text) {
        defineTool({
            name,
            capability: 'test:read',
            args: z.object({}),
    ...
    async function callPrincipal(name, transport) {
        const tool = lookupByMcpName(name);
        const r = await dispatchProjectedTool(tool, name, {}, ctx({
            transport,
        ...
        if (!r.ok)
            throw new Error(`dispatch failed: ${r.error?.code}`);
        return { text: r.result.content[0].text, meta: r.result._meta ?? {} };
    }
