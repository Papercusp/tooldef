...
import { parseDeltaRequest, formatDeltaRequest, encodeDeltaCursor, decodeDeltaCursor, computeViewFingerprint, negotiateDelta, computeRowDigest, computeViewChecksum, diffFromDigest, applySemanticDelta, deltaCounts, DELTA_MAX_DIGEST_ENTRIES, } from './delta-protocol';
const itemKey = (r) => r.id;
const rowRevision = (r) => r.rev;
...
describe('delta cursor encode/decode', () => {
    ...
    it('returns null (never throws) for malformed / invalid tokens', () => {
        ...
        expect(decodeDeltaCursor(encodeDeltaCursor({ v: 2, fp: 'a', rev: 'b' }))).toBeNull(); // wrong version
...
describe('computeViewFingerprint', () => {
    ...
    it('changes when args change', () => {
        expect(computeViewFingerprint({ ...base, args: { harness: 'papercusp', limit: 21 } })).not.toBe(computeViewFingerprint(base));
    });
    ...
    it('treats missing scope/format as empty (stable)', () => {
        expect(computeViewFingerprint({ toolName: 't', args: { a: 1 } })).toBe(computeViewFingerprint({ toolName: 't', args: { a: 1 }, scope: '', format: '' }));
    });
});
...
describe('computeViewChecksum', () => {
    ...
    it('is deterministic and order-insensitive', () => {
        expect(computeViewChecksum(rows, itemKey, rowRevision)).toBe(computeViewChecksum([...rows].reverse(), itemKey, rowRevision));
    });
...
describe('applySemanticDelta — merge-correctness (the D-008 de-risk property)', () => {
    ...
    it('a no-op delta (no changes) leaves the base identical', () => {
        const base = [{ id: 'a', name: 'A', rev: 1 }];
        const digest = computeRowDigest(base, itemKey, rowRevision);
        const changes = diffFromDigest(digest, base, itemKey, { rowRevision });
        expect(changes).toEqual([]);
        expect(applySemanticDelta(base, changes, (r) => r.id)).toEqual(base);
    });
});
