...
async function callWithUnknownKey(toolName) {
    ...
    const outcome = await dispatchProjectedTool(lookupByMcpName(toolName), toolName, { known: 'x', totallyUnknownArg: 'y' }, ctx(), DEPS);
    if (outcome.ok)
        throw new Error('expected the call to fail with invalid_input');
...
describe('unknownArgHint + server-vintage wiring', () => {
    ...
    it('a call that validates cleanly is completely unaffected by a registered resolver', async () => {
        ...
        const outcome = await dispatchProjectedTool(lookupByMcpName('test:vintage-hint-clean-call'), 'test:vintage-hint-clean-call', { known: 'x' }, ctx(), DEPS);
        expect(outcome.ok).toBe(true);
        expect(outcome.result.content[0].text).toBe('{"known":"x"}');
    });
    it('a top-level object type error renders that field shape instead of the oversized full schema', async () => {
        ...
        const outcome = await dispatchProjectedTool(lookupByMcpName('test:top-level-object-schema-hint'), 'test:top-level-object-schema-hint', { why: 'invalid scalar' }, ctx(), DEPS);
        if (outcome.ok)
            throw new Error('expected the call to fail with invalid_input');
