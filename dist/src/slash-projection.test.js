...
import { SLASH_PROMPT_PREFIX, deriveSlashPromptArguments, isSlashPromptName, renderSlashPrompt, resolveSlashExposure, slashPromptListingFor, slashPromptNameFor, slashPromptToolName, } from './slash-projection';
...
describe('resolveSlashExposure (default ON, D-003)', () => {
    ...
    it('returns the override object when given one', () => {
        const slash = { name: 'plans', description: 'd', args: ['slug'] };
...
describe('naming', () => {
    ...
    it('honors the slash name override', () => {
        expect(slashPromptNameFor(tool({ expose: { mcp: { name: 'plans:list' }, slash: { name: 'plans' } } }))).toBe('tool:plans');
    });
...
describe('slashPromptListingFor', () => {
    it('builds a listing with name, description (guidance.when preferred), and args', () => {
        const listing = slashPromptListingFor(tool({ guidance: { when: 'When the user asks about plans.' } }));
...
describe('renderSlashPrompt', () => {
    ...
    it('adds the hard confirm directive for high-tier capabilities (D-007)', () => {
        ...
        const text = renderSlashPrompt(tool({ capabilities: ['danger:write'] }), {}).messages[0].content.text;
    ...
    it('includes guidance when/notWhen lines when present', () => {
        const text = renderSlashPrompt(tool({ guidance: { when: 'For plan lookups.', notWhen: 'Not for features.' } }), {}).messages[0].content.text;
