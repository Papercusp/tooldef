...
import { dispatchProjectedTool, dispatchProjectedToolStream, defaultComputeQuotaWindow, InvalidInputError, } from './dispatch-projected';
...
describe('dispatchProjectedTool', () => {
    ...
    it('gateBypass.role skips the role-allowlist gate', async () => {
        ...
        const bypassed = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ role: 'worker', gateBypass: { role: true } }), MAKE_DEPS());
    ...
    it('gateBypass.capability skips the capability gate', async () => {
        ...
        const principal = {
            kind: 'system', slug: 'p', workspaceId: 'default',
            authMethod: 'process-internal', trust: 'trusted',
            capabilities: new Set(), // lacks 'secrets:read'
        };
        ...
        const bypassed = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal, gateBypass: { capability: true } }), MAKE_DEPS());
        expect(bypassed.ok).toBe(true);
    });
    it('appends host-provided route guidance without changing the denial', async () => {
        ...
        const denied = await dispatchProjectedTool(tool, 'plans:audit', {}, MAKE_CTX({ transport: 'mcp', role: 'judge', principal }), MAKE_DEPS({
            authorizationFailureHint: ({ toolName, missingCapability }) => `Use papercusp-su tools:invoke for ${toolName} (${missingCapability}).`,
        }));
    ...
    it("treats '*' as the canonical wildcard capability", async () => {
        const tool = makeTool({ capabilities: ['secrets:read'] });
        const principal = {
            kind: 'system', slug: 'system:operator', workspaceId: 'default',
            authMethod: 'process-internal', trust: 'trusted',
            capabilities: new Set(['*']),
        };
        const result = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal }), MAKE_DEPS());
    ...
    it('gateBypass.quota skips the quota gate', async () => {
        ...
        const bypassed = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ gateBypass: { quota: true } }), deps);
        expect(bypassed.ok).toBe(true);
    });
    ...
    it('gateBypass.harness skips the harness gate (explicit escape hatch)', async () => {
        ...
        const bypassed = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ harnessSlug: undefined, gateBypass: { harness: true } }), MAKE_DEPS());
        expect(bypassed.ok).toBe(true);
    });
    ...
    it('preserves registry provenance and deterministic corrections in dispatch + telemetry', async () => {
        ...
        let metadataJson;
        const result = await dispatchProjectedTool(lookupByMcpName('test:invalid-input-provenance'), 'test:invalid-input-provenance', { id: 'WI-1', topic: 'ops', tags: ['drift'] }, MAKE_CTX(), MAKE_DEPS({
    ...
    it('returns timeout when fn exceeds timeoutSec', async () => {
        const tool = makeTool({
            timeoutSec: 0.05,
            fn: async (_input, ctx) => new Promise((resolve, reject) => {
    ...
    it('aborts the handler when idleTimeoutSec elapses without an emit', async () => {
        const tool = makeTool({
            ...
            fn: (_input, ctx) => new Promise((resolve, reject) => {
...
describe('metadata_json auto-fill from ctx.uiClientId (telemetry-tagging audit)', () => {
    ...
    it('auto-fills metadata_json.uiClientId when handler never calls ctx.metadata', async () => {
        ...
        await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ uiClientId: 'audit-probe-1' }), MAKE_DEPS({
    ...
    it('merges ctx.uiClientId into handler-supplied metadata', async () => {
        ...
        await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ uiClientId: 'audit-probe-2' }), MAKE_DEPS({
            recordInvocation: async (input) => { captured = input.metadataJson; },
        }));
    ...
    it('respects handler override of uiClientId (does not clobber)', async () => {
        ...
        await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ uiClientId: 'audit-probe-3' }), MAKE_DEPS({
            recordInvocation: async (input) => { captured = input.metadataJson; },
        }));
    ...
    it('does nothing when ctx.uiClientId is null/undefined', async () => {
        ...
        await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ uiClientId: null }), MAKE_DEPS({
            recordInvocation: async (input) => { captured = input.metadataJson; },
        }));
    ...
    it('auto-fill applies on error path too', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ uiClientId: 'audit-probe-error' }), MAKE_DEPS({
            recordInvocation: async (input) => { captured = input.metadataJson; },
        }));
        expect(r.ok).toBe(false);
...
describe('overrideTool dep (plan §10.4 — brain-subprocess injection)', () => {
    ...
    it('overrideTool receives tool name + args + ctx for decisioning', async () => {
        const seen = [];
        const tool = makeTool();
        await dispatchProjectedTool(tool, 'harness.status', { slug: 'sheets' }, MAKE_CTX({ uiClientId: 'llm-testing/abc' }), MAKE_DEPS({
...
describe('authorize gate (RFC tooldef-auth Phase 1b — resource authz + audited break-glass)', () => {
    it('allow → handler runs, the allow is audited', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX(), MAKE_DEPS({ auditAuth: (e) => events.push(e) }));
        expect(r.ok).toBe(true);
    ...
    it('gateBypass.policy skips the hook BUT audits the bypass', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ gateBypass: { policy: true } }), MAKE_DEPS({ auditAuth: (e) => events.push(e) }));
    ...
    it('the role/capability/quota bypasses do NOT imply the authorize bypass', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ gateBypass: { role: true, capability: true, quota: true } }), MAKE_DEPS({ auditAuth: (e) => events.push(e) }));
        expect(r.ok).toBe(false);
        expect(r.error?.code).toBe('authorization_denied');
        expect(events[0]).toMatchObject({ gate: 'authorize', decision: 'deny' });
    });
...
describe('role-requirement gate (RFC tooldef-auth Phase 2 — declarative RBAC)', () => {
    ...
    it('allows when the principal has one of the required roles (any-of)', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal: withRoles(['staff', 'admin']) }), MAKE_DEPS());
        expect(r.ok).toBe(true);
    });
    it('denies (missing_role) + audits when the principal lacks every required role', async () => {
        const events = [];
        const tool = makeTool({ requireRoles: ['admin'] });
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal: withRoles(['staff']) }), MAKE_DEPS({ auditAuth: (e) => events.push(e) }));
        expect(r.ok).toBe(false);
    ...
    it('gateBypass.role bypasses the RBAC requirement (a superuser passes role gates)', async () => {
        const tool = makeTool({ requireRoles: ['admin'] });
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal: withRoles([]), gateBypass: { role: true } }), MAKE_DEPS());
        expect(r.ok).toBe(true);
    });
    it('a tool with no requireRoles is unaffected (additive)', async () => {
        const tool = makeTool();
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ principal: withRoles([]) }), MAKE_DEPS());
        expect(r.ok).toBe(true);
    });
});
describe('default-deny gate (RFC tooldef-auth Phase 3 — opt-in fail-closed posture)', () => {
    ...
    it('on: a tool gated only by agent roles is allowed (declares a gate)', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'fix.tool', {}, MAKE_CTX({ role: 'worker' }), MAKE_DEPS({ defaultDeny: true }));
        expect(r.ok).toBe(true);
    });
});
