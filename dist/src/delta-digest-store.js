 ...
 */
export function putRowDigest(digest, fp, rows) {
 ...
 */
export function getRowDigest(id, fp) {
    const entry = store.get(id);
    if (!entry)
        return undefined;
    if (entry.fp !== fp)
        return undefined; // cross-view id reuse — see safety property 2
    return entry.digest;
}
/** Test seam + a hook for a host that wants to drop the cache (e.g. on reload). */
export function resetRowDigestStore() {
    store.clear();
    retainedRows = 0;
    seq = 0;
}
/** Observability: current retention, for tests and host telemetry. */
export function rowDigestStoreStats() {
