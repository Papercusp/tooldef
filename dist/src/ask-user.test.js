...
import { _cardCorrelatorStatsForTests, _resetCardCorrelatorForTests, resolveCardResponse, } from './card-correlator';
import { _resetStateChannelForTests, getSnapshot } from './state-channel';
import { RESERVED_EVENT_NAMES, registerProjectedTool, _resetProjectionRegistryForTests, } from './tool-projection';
const choiceSchema = z.object({ picks: z.array(z.string()).min(1) });
function makeAskTool() {
    return {
        pluginName: 'test',
        description: 'ask',
        capabilities: [],
...
describe('ctx.askUser dispatcher integration', () => {
    ...
    it('askUser is wired when ctx has workspaceId + runId; resolves on /card-response submit', async () => {
        ...
        const dispatchPromise = dispatchProjectedTool(tool, 'test:ask', {}, MAKE_CTX({ runId: 'r1' }), MAKE_DEPS());
        ...
        if (result.ok && result.result) {
            const c = result.result.content?.[0]?.text;
    ...
    it('onCard fires with the freshly-minted correlationId (P-032 — Phase D link hook)', async () => {
        ...
        const dispatchPromise = dispatchProjectedTool(tool, 'test:ask', {}, MAKE_CTX({ runId: 'r-oncard' }), MAKE_DEPS());
        await new Promise((r) => setTimeout(r, 10));
    ...
    it('askUser is NOT installed when ctx lacks runId — handler sees ctx.askUser undefined', async () => {
        const tool = makeAskTool();
        const result = await dispatchProjectedTool(tool, 'test:ask', {}, MAKE_CTX({ runId: undefined }), MAKE_DEPS());
        expect(result.ok).toBe(true);
        if (result.ok && result.result) {
            const c = result.result.content?.[0]?.text;
    ...
    it('askUser resolves cancel when dispatcher aborts the run; pending card cleaned up', async () => {
        ...
        const tool = { ...makeAskTool(), timeoutSec: 0.05 };
        const result = await dispatchProjectedTool(tool, 'test:ask', {}, MAKE_CTX({ runId: 'r-cancel' }), MAKE_DEPS());
        expect(result).toBeDefined();
        expect(_cardCorrelatorStatsForTests().pendingCount).toBe(0);
    });
    it('"card" is a reserved event name (H1)', () => {
        expect(RESERVED_EVENT_NAMES).toContain('card');
    });
