...
import { registerPrompt, lookupPrompt, getPromptCatalog, _resetPromptCatalogForTests, } from './prompt-registry';
const baseDef = (over = {}) => ({
    name: 'review:summary',
...
describe('collision handling', () => {
    ...
    it('throws on a same-name collision with a different capability', () => {
        ...
        expect(() => registerPrompt(baseDef({ capability: 'review:write' }))).toThrow(/Prompt name collision: "review:summary"/);
    });
    it('treats undefined vs a real capability as a real collision', () => {
        registerPrompt(baseDef({ capability: undefined }));
        expect(() => registerPrompt(baseDef({ capability: 'review:read' }))).toThrow(/collision/);
    });
});
