...
const card = (correlationId, extra = {}) => ({ correlationId, id: `card:${correlationId}`, ...extra });
const snap = (version, openCards, toolState) => ({
    runId: 'r1',
    version,
...
describe('chooseSnapshotEmission (stream full-vs-delta policy)', () => {
    ...
    it('emits a delta to a delta-aware consumer once a base exists', () => {
        ...
        expect(applySnapshotDelta(prev, out.data)).toEqual(next); // the delta reconstructs next
    });
