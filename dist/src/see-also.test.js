...
import { resolveSeeAlso, renderSeeAlsoText, readJsonResult, applySeeAlso, } from './see-also';
...
describe('readJsonResult', () => {
    it('parses the first text content block as JSON', () => {
        expect(readJsonResult(textResult({ ok: true }))).toEqual({ ok: true });
    });
    it('returns undefined when the first text block is not JSON', () => {
        expect(readJsonResult({ content: [{ type: 'text', text: 'not json' }] })).toBeUndefined();
    });
...
describe('applySeeAlso', () => {
    ...
    it('passes a soft-error result through unchanged (failed calls pay nothing)', () => {
        const err = { content: [{ type: 'text', text: '{}' }], isError: true };
    ...
    it('preserves pre-existing _meta keys', () => {
        const base = { content: [{ type: 'text', text: '{}' }], _meta: { keep: 1 } };
