...
describe('register', () => {
    ...
    it('throws on a same-name collision with a different capability', () => {
        register(baseDef());
        expect(() => register(baseDef({ capability: 'other:write' }))).toThrow(/capability=fix:read/);
    });
    ...
    it('throws on a same-name same-capability collision between DIFFERENT tools (EI-14)', () => {
        ...
        expect(() => register(baseDef({ name: 'coord:ask', capability: 'coord:write', description: 'ask the owner' }))).toThrow(/silently shadows the first/);
    });
    it('silently replaces a structurally-identical re-eval (HMR / double-import)', () => {
        ...
        expect(() => register(baseDef({ name: 'coord:ask', capability: 'coord:write', description: 'knowledge-first' }))).not.toThrow();
