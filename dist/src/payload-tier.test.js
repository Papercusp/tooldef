...
import { applyPayloadTier, extractPayloadTier, parsePayloadTier, resolvePayloadTier, resetPayloadTierRatchet, projectBoundedPayload, PAYLOAD_TIER_RATCHET_CHARS, PAYLOAD_TIER_HARD_CEILING_CHARS, } from './payload-tier';
...
describe('applyPayloadTier', () => {
    ...
    const response = { data };
    const shape = {
        standard: (d) => ({ ...d, detail: undefined, tierMark: 'standard' }),
        trimmed: (d, sctx) => ({ rows: d.rows.slice(0, 1), tierMark: sctx.tier }),
    };
    ...
    it('hard ceiling: a full payload over the ceiling force-applies the trimmed shaper (WI-2859)', () => {
        const log = vi.fn();
        const fat = { data: { rows: [1, 2, 3], blob: 'x'.repeat(PAYLOAD_TIER_HARD_CEILING_CHARS + 500) } };
        ...
        expect(out.data.tierMark).toBe('trimmed');
        // …and the forced downgrade is marked so the caller knows.
        expect(out.data.payloadTierForced).toBe('trimmed');
    ...
    it('ceilingChars: a per-tool ceiling lets an over-DEFAULT payload through unshaped', () => {
        ...
        expect(out).toBe(fat);
        expect(out.data.payloadTierForced).toBeUndefined();
    ...
    it('ceilingChars: a raised ceiling is a valve, not a bypass — past IT the trimmed shaper still fires', () => {
        ...
        const fat = { data: { rows: [1, 2, 3], blob: 'x'.repeat(raised + 500) } };
        const out = applyPayloadTier({
            toolName: 'coord:orient',
            shape,
            response: fat,
    ...
    it('hard ceiling: an over-ceiling payload with NO shaper gets a loud bounded projection', () => {
        const log = vi.fn();
        const fat = { data: { blob: 'y'.repeat(PAYLOAD_TIER_HARD_CEILING_CHARS + 500) } };
        const out = applyPayloadTier({ toolName: 't', shape: undefined, response: fat, tier: 'full', args: {}, log });
        const projected = out.data;
    ...
    it('hard ceiling falls back to the generic projection when a custom shape does not shrink', () => {
        const log = vi.fn();
        const identityShape = { trimmed: (d) => d };
        const fat = { data: { blob: 'z'.repeat(PAYLOAD_TIER_HARD_CEILING_CHARS + 500) } };
        const out = applyPayloadTier({ toolName: 't', shape: identityShape, response: fat, tier: 'full', args: {}, log });
        expect(out.data._projection.forced).toBe(true);
        expect(JSON.stringify(out.data).length).toBeLessThan(PAYLOAD_TIER_HARD_CEILING_CHARS);
        expect(log).toHaveBeenCalledWith(expect.stringContaining('hard ceiling'));
    });
    it('trimmed picks shape.trimmed; standard picks shape.standard', () => {
        ...
        expect(s.data.tierMark).toBe('standard');
    });
    it('trimmed falls back to standard when no trimmed shaper; to unshaped when neither', () => {
        ...
        expect(viaStd.data.tierMark).toBe('standard');
    ...
    it('ratchet: a fat unshaped payload warns once and is auto-projected with exact re-fetch guidance', () => {
        const log = vi.fn();
        const fat = { data: { blob: 'x'.repeat(PAYLOAD_TIER_RATCHET_CHARS + 100) } };
    ...
    it('wraps top-level arrays so bounded output always carries omission metadata', () => {
        ...
        expect(projected.items.length).toBeLessThan(original.length);
        expect(projected._projection.omittedCount).toBeGreaterThan(0);
    ...
    it('can preserve a top-level array root while keeping projection metadata in memory', () => {
        ...
        expect(JSON.parse(JSON.stringify(projected))).toEqual(Array.from(projected));
        expect(original).toHaveLength(200);
    });
    it('accepts an exact transport target and a schema-valid host recovery cursor', () => {
        ...
        const projected = projectBoundedPayload({ rows: Array.from({ length: 100 }, (_, i) => ({ id: `WI-${i}`, detail: 'x'.repeat(500) })) }, { toolName: 'work_items:list', tier: 'trimmed', targetChars: 5_000, recovery });
    ...
    it('retains identity for every deeply nested recipe-result row and returns a callable continuation (EI-11404)', () => {
        ...
        const rows = (projected.results[0].result.summary);
        expect(rows).toHaveLength(10);
        expect(rows.every((row) => typeof row !== 'string')).toBe(true);
        expect(rows.map((row) => row.results[0].id)).toEqual(Array.from({ length: 10 }, (_, i) => `WI-${4600 + i}`));
        expect(projected._projection.cursor).toEqual({
            kind: 'full-detail',
            tool: 'recipes:run',
    ...
    it('REGRESSION (EI-21974854722078500): an explicit nested pick survives key/depth fallback while an unselected sibling stays partial', () => {
        ...
        const noise = (prefix) => Object.fromEntries(Array.from({ length: 48 }, (_, i) => [`${prefix}-${i}`, `noise-${i}`]));
        const payload = {
            results: [{
                    ...noise('result-noise'),
                    rubric: {
        ...
        const row = projected.results[0].rubric.criteria[0];
    ...
    it('REGRESSION (EI-21197620758075816): rubric criterion check bindings survive the depth preview', () => {
        ...
        const projected = projectBoundedPayload({ results: [{ ok: true, rubric: { rubricId: 'bound-rubric', criteria } }] }, { toolName: 'rubrics:get', tier: 'trimmed' });
        const rows = (projected.results[0].rubric.criteria);
        expect(rows).toHaveLength(8);
        expect(rows.slice(0, 5).map((row) => row.check)).toEqual(Array.from({ length: 5 }, (_, i) => ({
            kind: 'tests',
    ...
    it("REGRESSION (EI-21364690783677984): a rubric criterion's grading METHOD survives the depth preview, and a surviving one is never silently partial", () => {
        ...
        const projected = projectBoundedPayload({ results: [{ ok: true, rubric: { rubricId: 'graded-rubric', criteria } }] }, { toolName: 'rubrics:get', tier: 'trimmed' });
        const rows = (projected.results[0].rubric.criteria);
    ...
    it('retains every bulk row id and marks budget-partial objects in band (EI-18683546971375407 / EI-21566007420787313)', () => {
        ...
        for (const row of rows) {
            ...
            if (rootLostField)
                expect(row._partial).toBe(true);
            if (workItemLostField)
                expect(workItem?._partial).toBe(true);
        }
        ...
        expect(rows.some((row) => typeof row !== 'string' &&
            row._truncated !== true &&
            (row._partial === true || row.workItem?._partial === true))).toBe(true);
...
describe('payloadProjection meta (EI-13918)', () => {
    ...
    it('is stamped on the hard-ceiling forced-custom-shaper path (WI-2859), even though no inline `_projection` rides the shaped data', () => {
        const shape = { trimmed: (d) => ({ tierMark: 'trimmed', rows: d.rows }) };
        const fat = { data: { rows: [1, 2, 3], blob: 'x'.repeat(PAYLOAD_TIER_HARD_CEILING_CHARS + 500) } };
    ...
    it('is ABSENT for a normal-size / unshaped / explicit-full response (no false positives)', () => {
        ...
        const fat = { data: { blob: 'x'.repeat(PAYLOAD_TIER_HARD_CEILING_CHARS + 500) } };
        expect(applyPayloadTier({ toolName: 't', shape: undefined, response: fat, tier: 'full', explicitFullRequest: true, args: {} })
            .payloadProjection).toBeUndefined();
    });
});
/**
...
describe('projectBoundedPayload — truncation is announced in band', () => {
    ...
    it('leaves a string that FITS byte-identical — the marker must not become noise', () => {
        ...
        expect(out.note).not.toMatch(/TRUNCATED/);
    });
    it('marks AND records the BUDGET-pressure clip, which was previously silent in both channels', () => {
        ...
        const rows = Object.fromEntries(Array.from({ length: 40 }, (_, i) => [`f${i}`, 'B'.repeat(700)]));
        const out = projectBoundedPayload(rows, { toolName: 't', tier: 'trimmed' });
        const clipped = Object.entries(out).filter(([k, v]) => k !== '_projection' && typeof v === 'string' && /TRUNCATED/.test(v));
    ...
    it('REGRESSION: a checkpoint-sized string never comes back looking complete', () => {
        ...
        const out = projectBoundedPayload({ ok: true, results: [{ ok: true, id: 'WI-6638', checkpoint }] }, { toolName: 'work_items:get', tier: 'trimmed', args: { id: 'WI-6638', detail: true } });
        const got = out.results[0].checkpoint;
    ...
    it("cursor advice states that payloadTier is framework-reserved, not schema-passable", () => {
        const out = projectBoundedPayload({ blob: 'z'.repeat(20_000) }, { toolName: 'work_items:get', tier: 'trimmed', args: { id: 'WI-1' } });
    ...
    it('bounds large request args instead of re-expanding them in the recovery cursor', () => {
        ...
        const out = projectBoundedPayload({ ok: true, count: args.messages.length, error: null }, { toolName: 'coord:send', tier: 'trimmed', args });
    ...
    it('renders the host rawDispatchTemplate as a concrete executable call in `next`', () => {
        const out = projectBoundedPayload({ blob: 'z'.repeat(20_000) }, {
            toolName: 'work_items:get',
            tier: 'trimmed',
            args: { id: 'WI-1' },
    ...
    it('both truncated-args and normal branches close with the ask-for-LESS lead', () => {
        ...
        const normal = projectBoundedPayload({ blob: 'z'.repeat(20_000) }, { toolName: 'work_items:get', tier: 'trimmed', args: { id: 'WI-1' }, rawDispatchTemplate: template });
        const bigArgs = {
            messages: Array.from({ length: 67 }, (_, i) => ({
                to: [`agent-${i}`],
                body: [{ text: 'x'.repeat(500) }],
        ...
        const truncatedArgs = projectBoundedPayload({ ok: true, count: 67, error: null }, { toolName: 'coord:send', tier: 'trimmed', args: bigArgs, rawDispatchTemplate: template });
...
describe('projectBoundedPayload — a value dropped WHOLE says how much and how to recover it', () => {
    ...
    describe('when a durable recovery cursor exists, the marker points THERE', () => {
        ...
        const assertPointsAtCursor = (markers) => {
            expect(markers.length).toBeGreaterThan(0);
            for (const m of markers) {
        ...
        const markersOf = (out) => [...new Set(JSON.stringify(out).match(/\[omitted:[^\]"]*\]/g) ?? [])];
        it('every omission marker names the cursor, never the inert re-call', () => {
            assertPointsAtCursor(markersOf(projectBoundedPayload(dropping, {
                toolName: 't',
                tier: 'trimmed',
                forced: true,
                recovery: RECOVERY,
        ...
        it('DISCRIMINATOR: the SAME payload through the SAME projector yields DIFFERENT pointers', () => {
            ...
            const withCursor = markersOf(projectBoundedPayload(dropping, { toolName: 't', tier: 'trimmed', forced: true, recovery: RECOVERY }));
    ...
    it('REGRESSION (EI-19965559011729712): an array truncated at the maxArray cap announces the drop IN BAND and in omitted[], never starved by field-level entries', () => {
        ...
        const out = projectBoundedPayload({ rubric: { rubricId: 'goal-mode-e2e', criteria } }, { toolName: 'rubrics:get', tier: 'trimmed' });
        const projectedCriteria = out.rubric.criteria;
        ...
        expect(marker.note).toMatch(/TRUNCATED \+\d+ more item\(s\)/);
        expect(marker.note).toContain('header count includes this marker');
        expect(marker.note).toContain('showing 12 of 17');
    ...
    it('REGRESSION (EI-21219452028880493): the marker matches the PROJECTED array, not the SOURCE — a non-object element beyond the shown window must not downgrade an object-row marker to a string', () => {
        ...
        const projected = out.active_locks;
    ...
    it('keeps the scalar in-band marker for primitive arrays', () => {
        const out = projectBoundedPayload({ values: Array.from({ length: 17 }, (_, i) => i) }, { toolName: 't', tier: 'trimmed' });
        const marker = out.values.at(-1);
        expect(typeof marker).toBe('string');
        expect(marker).toContain('showing 12 of 17');
    });
