...
import { configurePrePromptRegistry, clearPrePromptRegistry, advertisedArgsSchema, listPrePromptEntries, projectReadColumns, projectWriteColumns, renderWireSchemas, reconstructArgs, } from '@papercusp/result-encoding';
import { lookupByMcpName, listMcpProjections, _resetProjectionRegistryForTests, } from './tool-projection';
const DEPS = {};
const ctx = (over = {}) => ({
    log: vi.fn(),
...
function defineBulkSetState(name) {
    let received;
    defineTool({
        name,
        requirePrincipal: false,
...
describe('positional write shim — end-to-end (P-008/P-009)', () => {
    it('advertises a single `row` string for a registry write-positional tool', () => {
        ...
        const advertised = advertisedArgsSchema('wi:set_state', listing.inputSchema);
        const props = advertised.properties ?? {};
        expect(Object.keys(props)).toEqual(['row']);
        expect(props.row.description).toContain('id, state, harness?');
    });
    it('reconstructs a positional `{ row }` into typed args before the handler', async () => {
        ...
        const r = await dispatchProjectedTool(lookupByMcpName('wi:set_state2'), 'wi:set_state2', { row: 'WI-12,passed' }, ctx(), DEPS);
    ...
    it('GUARD: a misaligned row (bad enum) is rejected, not silently written', async () => {
        ...
        const r = await dispatchProjectedTool(lookupByMcpName('wi:set_state3'), 'wi:set_state3', { row: 'WI-12,not-a-state' }, ctx(), DEPS);
    ...
    it('tolerates the natural `WI-12, passed` spacing end-to-end (trim normalization)', async () => {
        ...
        const r = await dispatchProjectedTool(lookupByMcpName('wi:set_state_ws'), 'wi:set_state_ws', { row: 'WI-12, passed' }, // space after the comma
        ctx(), DEPS);
        expect(r.ok).toBe(true);
        expect(tool.received()).toEqual({ id: 'WI-12', state: 'passed' });
    });
    it('keyed args still work for a non-prompt-aware caller', async () => {
        ...
        const r = await dispatchProjectedTool(lookupByMcpName('wi:set_state4'), 'wi:set_state4', { id: 'WI-7', state: 'failing' }, ctx(), DEPS);
        expect(r.ok).toBe(true);
    ...
    it('bulk-capable positional tools advertise row+keyed oneOf and dispatch both shapes', async () => {
        ...
        const advertised = advertisedArgsSchema('wi:bulk_set_state', listing.inputSchema);
        const oneOf = advertised.oneOf;
        expect(oneOf).toHaveLength(2);
        expect((oneOf[0].properties?.row).description).toContain('id, state, harness?');
        expect(oneOf[1].properties).toHaveProperty('ids');
        const row = await dispatchProjectedTool(lookupByMcpName('wi:bulk_set_state'), 'wi:bulk_set_state', { row: 'WI-12,passed' }, ctx(), DEPS);
        expect(row.ok).toBe(true);
        expect(tool.received()).toEqual({ id: 'WI-12', state: 'passed' });
        const keyedBulk = await dispatchProjectedTool(lookupByMcpName('wi:bulk_set_state'), 'wi:bulk_set_state', { ids: ['WI-1', 'WI-2'], state: 'failing' }, ctx(), DEPS);
    ...
    it('trailing free-text keeps embedded commas through the full path', async () => {
        ...
        const r = await dispatchProjectedTool(lookupByMcpName('wi:comment'), 'wi:comment', { row: 'WI-9,fixed it, added a test, shipped' }, ctx(), DEPS);
        expect(r.ok).toBe(true);
...
describe('anti-desync: legend columns === wire columns (P-011)', () => {
    it('the prompt legend, the read serializer, and the write shim share one projection', async () => {
        ...
        const legend = renderWireSchemas(listPrePromptEntries(), (e) => e.name === 'wi:rlist'
            ? { read: projectReadColumns(rTool.outputJsonSchema) }
            : { write: projectWriteColumns(wTool.inputSchema) });
        ...
        expect(rr.result.content[0].text).toBe('format: csv\n[1]\nWI-1,todo');
        // WRITE: the shim reconstructs by the SAME column order, end-to-end.
        const writeCols = projectWriteColumns(wTool.inputSchema);
...
describe('robustness — no collateral damage + graceful failure', () => {
    it('a NON-registry tool is byte-identical with the registry configured (no read/write/advertise change)', async () => {
        ...
        expect(advertisedArgsSchema('rb:plainlist', listing.inputSchema)).toEqual(listing.inputSchema);
    });
    it('Tier-3 SAFE FALLBACK: a flat-schema read tool whose RUNTIME row has a nested field → TOON, not broken CSV', async () => {
        defineTool({
            ...
            handler: async () => ({ data: [{ id: 'a', state: 'todo', extra: { nested: 1 } }] }),
        });
    ...
    it('a MISCONFIGURED registry entry (non-fitting tool marked positional) degrades to keyed safely', async () => {
        ...
        const advertised = advertisedArgsSchema('rb:arrayarg', listing.inputSchema);
        expect(advertised.properties).toHaveProperty('to');
    ...
    it('advertisedArgsSchema is idempotent (double application stays {row})', () => {
        ...
        const once = advertisedArgsSchema('rb:idem', listing.inputSchema);
        const twice = advertisedArgsSchema('rb:idem', once);
        expect(Object.keys(twice.properties ?? {})).toEqual(['row']);
    });
});
...
describe('needsWorkspaceTx projection parity (EI-18808330244321407)', () => {
    it.each([
        ['principal', {}],
        ['role', { requirePrincipal: false }],
    ])('a %s-gated defineTool preserves the explicit tx opt-in', (_kind, gate) => {
        const name = `wi:tx_${_kind}`;
        defineTool({
            name,
            ...gate,
        ...
        expect(lookupByMcpName(name)?.needsWorkspaceTx).toBe(true);
    });
    it('defaults to transaction-free when the declaration is absent', () => {
        defineTool({
            name: 'wi:tx_default',
            capability: 'test:write',
            args: z.object({}),
        ...
        expect(lookupByMcpName('wi:tx_default')?.needsWorkspaceTx).toBeFalsy();
    });
});
