...
const projected = (mcpName) => listAllProjectedTools().find((p) => p.expose.mcp?.name === mcpName);
...
describe('defineTool schema projections', () => {
    it('keeps union requirements for discovery without weakening the MCP/OpenAI schema', () => {
        ...
        expect(projected?.discoveryInputSchema?.anyOf).toEqual(expect.arrayContaining([
