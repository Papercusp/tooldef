...
function sourceFiles(dir) {
    ...
    for (const name of readdirSync(dir)) {
        ...
        if (/\.test\.tsx?$/.test(name))
            continue; // tests run under Node only
        out.push(p);
    }
    return out;
}
...
describe('tooldef stays browser-safe (isomorphic barrel)', () => {
    it('no non-test source file statically imports a node: builtin', () => {
        ...
        for (const file of sourceFiles(SRC)) {
            ...
            if (STATIC_NODE_IMPORT.test(code))
                offenders.push(path.relative(SRC, file));
        }
        expect(offenders, `Static node: imports leak into the browser via the barrel and blank the SPA.\n` +
            `Load them lazily (await import('node:…') inside the fn) or as import type.\n` +
            `Offenders: ${offenders.join(', ')}`).toEqual([]);
    });
});
