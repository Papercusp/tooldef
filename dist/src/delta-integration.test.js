...
import { decodeDeltaCursor, encodeDeltaCursor, computeViewChecksum, applySemanticDelta, setSemanticDeltaEnabledResolver, resetSemanticDeltaEnabledResolver, DELTA_MAX_DIGEST_ENTRIES, } from './delta-protocol';
import { resetRowDigestStore, rowDigestStoreStats, getRowDigest, putRowDigest, DIGEST_STORE_MAX_ROWS, } from './delta-digest-store';
const semItemKey = (r) => r.id;
const semRowRevision = (r) => r.rev;
const sortById = (rs) => [...rs].sort((a, b) => a.id.localeCompare(b.id));
/** Parse a serialized tool body back to its value (handles the `format: <fmt>` marker). */
function parseBody(text) {
    const m = /^format: (\w+)\n/.exec(text);
    if (!m)
        return JSON.parse(text);
    return decode(text.slice(m[0].length), m[1]);
}
// A bounded heterogeneous-free row-set, comfortably over the bypass threshold.
function makeSemRows(n = 10) {
...
function defineSemanticTool(name, state, opts = {}) {
    defineTool({
        name,
...
function defineDeltaTool(name, revRef, opts = {}) {
    defineTool({
        name,
        requirePrincipal: false,
        capability: 'test:read',
...
describe('semantic deltas end-to-end (Lane E)', () => {
    ...
    it('changed view → mode:delta carrying ONLY the changed rows + counts + checksum; merge reconstructs the view', async () => {
        ...
        const baseRows = parseBody(first.text);
        ...
        const changes = parseBody(second.text);
...
describe('large views still delta via a server-held digest (P-024)', () => {
    ...
    it('THE FIX: a changed over-cap view serves mode:delta with only the changed rows', async () => {
        ...
        const merged = applySemanticDelta(parseBody(first.text), changes, semItemKey);
        expect(sortById(merged)).toEqual(sortById(state.rows));
...
describe('grouped-view exemplar (the plans:attention pattern — rows selector flattens groups)', () => {
    ...
    function defineGroupedTool(name, state) {
        defineTool({
            name,
            requirePrincipal: false,
            capability: 'test:read',
    ...
    it('an item changed inside a group → delta carrying the changed items; merge over the flattened base reconstructs it', async () => {
        ...
        const baseItems = flatten(parseBody(first.text).groups);
...
describe('host flag gate (FLAGS.TOOL_DELTA_PROTOCOL via setSemanticDeltaEnabledResolver)', () => {
    ...
    it('resolver OFF: a changed view that WOULD delta degrades to full + reason:flag_off', async () => {
        ...
        expect(sortById(parseBody(second.text))).toEqual(sortById(state.rows));
    });
