...
import { computeViewChecksum, setSemanticDeltaEnabledResolver, resetSemanticDeltaEnabledResolver, } from './delta-protocol';
 ...
 */
function defineSemanticTool(name, state) {
    defineTool({
        name,
...
function toDeltaResponse(result) {
    const meta = (result._meta ?? {});
    const d = meta.delta ?? { mode: 'full' };
    const text = result.content[0]?.text ?? '';
    if (d.mode === 'not_modified')
        return { mode: 'not_modified', cursor: d.cursor };
    if (d.mode === 'delta') {
        return { mode: 'delta', cursor: d.cursor, checksum: d.checksum, changes: parseBody(text) };
    }
    return { mode: 'full', cursor: d.cursor, rows: parseBody(text) };
}
/** A real in-process dispatch for `name`, threading the negotiated `_meta.delta` verbatim. */
function realDispatch(name) {
    return async (requested) => {
        const tool = lookupByMcpName(name);
        const r = await dispatchProjectedTool(tool, name, {}, baseCtx(requested === undefined ? {} : { requestedDelta: requested }), DEPS);
        if (!r.ok || !r.result)
            throw new Error(`dispatch failed: ${r.error?.code}`);
        return toDeltaResponse(r.result);
    };
}
...
describe('delta client round-trip (BasePresenceTracker + DeltaToolClient ↔ real dispatchProjectedTool)', () => {
    ...
    it('cold read → mode:full, full snapshot reconstructed, base now tracked', async () => {
        ...
        expect(sortById(out.rows)).toEqual(sortById(state.rows));
    ...
    it('out-of-scope harness (enabled:false — external Claude Code/Codex) ⇒ EVERY read is full, never not_modified/delta', async () => {
        ...
        expect(sortById(b.rows)).toEqual(sortById(state.rows));
    });
    it('GAP-DETECTOR: across a realistic poll loop, re-reads actually engage the no-replay path (not silently always-full)', async () => {
        ...
        for (let i = 0; i < 6; i += 1) {
            if (i === 3)
                state.rows = [...state.rows, { id: `extra-${i}`, name: 'fresh-row-padding-text' }];
            const out = await dispatchWithBasePresence(tracker, client, VIEW, semItemKey, dispatch, { wantSemantic: true });
            modes.push(out.mode);
