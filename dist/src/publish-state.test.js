...
import { _resetStateChannelForTests, getSnapshot, openRun, } from './state-channel';
...
function makeStatefulTool(over) {
    return {
        pluginName: 'test',
...
describe('ctx.publishState dispatcher integration', () => {
    ...
    it('happy-path: snapshots flow through state-channel; final snapshot persists', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'test:search', {}, MAKE_CTX({ runId: 'r-ps' }), MAKE_DEPS());
    ...
    it('publishState is NOT installed when ctx lacks runId', async () => {
        const tool = makeStatefulTool();
        const r = await dispatchProjectedTool(tool, 'test:search', {}, MAKE_CTX({ runId: undefined }), MAKE_DEPS());
        expect(r.ok).toBe(true);
        if (r.ok) {
            expect(r.result?.content?.[0]?.text).toBe('no-publishState');
        }
    });
    it('publishState is NOT installed when tool has no state schema', async () => {
        const tool = makeStatefulTool({ state: undefined });
        const r = await dispatchProjectedTool(tool, 'test:search', {}, MAKE_CTX({ runId: 'r-ps2' }), MAKE_DEPS());
        expect(r.ok).toBe(true);
        if (r.ok) {
            expect(r.result?.content?.[0]?.text).toBe('no-publishState');
        }
        ...
        const tool = {
            ...
            fn: async (_input, ctx) => {
                try {
                    ...
                    ctx.publishState({ results: [], progress: 0 });
                    return { content: [{ type: 'text', text: 'no-throw' }] };
                }
                catch (e) {
                    return { content: [{ type: 'text', text: `threw:${e.message.slice(0, 30)}` }] };
                }
            },
        };
        openRun({ workspaceId: 'workspace-S', runId: 'r-bad' });
        const r = await dispatchProjectedTool(tool, 'test:bad', {}, MAKE_CTX({ runId: 'r-bad' }), MAKE_DEPS());
        expect(r.ok).toBe(true);
        if (r.ok) {
            expect(r.result?.content?.[0]?.text).toMatch(/^threw:/);
        }
    });
    it('M1: JSON-Patch array input rejected in v1 (snapshot-only)', async () => {
        const tool = {
            ...
            fn: async (_input, ctx) => {
                try {
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    ctx.publishState([{ op: 'replace', path: '/phase', value: 'done' }]);
                    return { content: [{ type: 'text', text: 'no-throw' }] };
                }
                catch (e) {
        ...
        const r = await dispatchProjectedTool(tool, 'test:patch', {}, MAKE_CTX({ runId: 'r-patch' }), MAKE_DEPS());
        expect(r.ok).toBe(true);
        if (r.ok) {
            expect(r.result?.content?.[0]?.text).toMatch(/snapshot-only/);
        }
    });
    it('M5: state-shaped tools opt OUT of the replay ring buffer at register-time', async () => {
        ...
        await dispatchProjectedTool(tool, 'test:search', {}, MAKE_CTX({ runId: 'r-m5' }), MAKE_DEPS());
    ...
    it('state outlives disconnect: snapshot persists after run-end (5min retention)', async () => {
        ...
        const r = await dispatchProjectedTool(tool, 'test:search', {}, MAKE_CTX({ runId: 'r-outlive' }), MAKE_DEPS());
        expect(r.ok).toBe(true);
