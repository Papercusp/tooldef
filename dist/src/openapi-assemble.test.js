...
describe('toolOperationName', () => {
    ...
    it('falls back to the HTTP path with slashes → dots', () => {
        expect(toolOperationName(makeTool({ expose: { http: { path: '/plugins/repomix/pack' } } }))).toBe('plugins.repomix.pack');
    });
    it('returns null when neither exposure is set', () => {
        expect(toolOperationName(makeTool({ expose: {} }))).toBeNull();
    });
});
describe('assembleOpenApiDocument', () => {
    it('emits a valid 3.1.0 envelope', () => {
        ...
        expect(doc.info.title).toBe('Papercusp Tool API');
    });
    ...
    it('emits one path per tool under .post', () => {
        ...
        expect(paths['/api/one.tool'].post).toBeDefined();
    });
    ...
    it('throws on a path collision', () => {
        ...
        expect(() => assembleOpenApiDocument([
    ...
    it('skips tools with no exposure (no mcp + no http)', () => {
        ...
        expect(Object.keys(doc.paths)).toEqual(['/api/real.tool']);
    });
