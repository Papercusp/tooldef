...
import { z } from 'zod';
import { analyzeSchema, configurePrePromptRegistry, clearPrePromptRegistry, projectReadColumns, } from '@papercusp/result-encoding';
import { serializeToolResponse, formatOptsFromCtx } from './serialize-result';
const flatRows = [
    { id: 1, name: 'a' },
    { id: 2, name: 'b' },
];
function elig(schema) {
...
describe('serializeToolResponse — format selection', () => {
    ...
    it('explicit csv is honored when the schema proves the data is flat', () => {
        ...
        expect(r.content[0].text).toBe('format: csv\nid,name\n1,a\n2,b');
    ...
    it('size-guard: a HETEROGENEOUS bulk envelope (TOON larger than JSON) is served as JSON, not the bigger TOON', () => {
        ...
        expect(r.content[0].text).toBe(JSON.stringify(envelope));
    });
});
describe('serializeToolResponse — envelope routing', () => {
    it('routes nextCursor/degraded into _meta, out of the body', () => {
        ...
        expect(r.content[0].text).not.toContain('CUR');
    });
    ...
    it('appends uiResources after the text item', () => {
        const ctx = { transport: 'mcp' };
        const ui = { type: 'resource', resource: { uri: 'ui://x', mimeType: 'text/html', text: '<b/>' } };
...
describe('serializeToolResponse — Tier-3 prompt-declared columns (read, P-004)', () => {
    ...
    it('explicit ?format=json opts OUT of Tier-3 (lossless honored)', () => {
        ...
        expect(r.content[0].text).toBe(JSON.stringify(rows));
    });
