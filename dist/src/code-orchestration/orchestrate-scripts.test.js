...
const run = (script, tools, opts = {}) => runToolOrchestration(script, {
...
describe('code:run acceptance — list-then-act-per-item loops', () => {
    it('"scan every open work item, return only the blocked ids" (loop + filter + compact return)', async () => {
        ...
        const r = await run(`const l = await tools.workItems.list({ status: 'open' });
       const blocked = [];
       for (const w of l.items) {
         const d = await tools.workItems.get({ id: w.id });
         if (d.state === 'blocked') blocked.push(d.id);
       }
       return { scanned: l.items.length, blocked };`, [list, get]);
    ...
    it('per-item try/catch — one failing item does not abort the rest', async () => {
        ...
        const get = mkTool('work_items:get', 'read', async (a) => {
            const id = a.id;
            if (id === 'BOOM')
                throw new Error('synthetic per-item failure');
            return json({ id, ok: true });
        });
        const r = await run(`const l = await tools.workItems.list({});
...
describe('code:run acceptance — only the summary re-enters context', () => {
    it('a 10k-row fetch is aggregated to a tiny summary; the rows never appear in the result', async () => {
        const fetch = mkTool('db:fetch', 'read', async () => json({ rows: Array.from({ length: 10_000 }, (_, i) => ({ i, v: i * 2 })) }));
        const r = await run(`const data = await tools.db.fetch({});
       let sum = 0; for (const row of data.rows) sum += row.v;
       return { count: data.rows.length, sum, max: data.rows[data.rows.length - 1].v };`, [fetch]);
        expect(r.ok).toBe(true);
...
describe('code:run acceptance — adversarial nested parallel fan-out (P-018)', () => {
    it('executes exactly 100 nested-parallel calls while keeping 35K+ of intermediate output out of context', async () => {
        ...
        const r = await run(`const groups = Array.from({ length: 10 }, (_, group) =>
...
describe('code:run acceptance — branching and retry control flow', () => {
    it('branches on a count threshold and only does the expensive path when warranted', async () => {
        ...
        const r = await run(`const l = await tools.issues.list({});
       if (l.items.length > 5) { await tools.coord.escalate({ summary: 'too many' }); return { escalated: true, n: l.items.length }; }
       return { escalated: false, n: l.items.length };`, [list, escalate]);
        expect(r.ok).toBe(true);
...
describe('code:run acceptance — facade surface', () => {
    it('tools.call("ns:verb") escape hatch dispatches the same as tools.ns.verb', async () => {
        ...
        const r = await run(`const viaCall = await tools.call('plans:set-now', { slug: 's', state: 'x' });
       return { viaCall };`, [tool], { dryRun: true });
        expect(r.ok).toBe(true);
...
describe('code:run acceptance — dry-run write gate', () => {
    it('dryRun: reads execute, writes are RECORDED in order but NOT executed', async () => {
       ...
       return { wouldUpdate: l.items.length };`, [list, setState], { dryRun: true });
        expect(r.ok).toBe(true);
    ...
    it('committing run: the SAME script now executes every write, still recording them for audit', async () => {
        ...
        const r = await run(`const l = await tools.workItems.list({});
       for (const w of l.items) await tools.workItems.setState({ id: w.id, state: 'done' });
       return { updated: l.items.length };`, [list, setState], { dryRun: false });
        expect(r.ok).toBe(true);
...
describe('code:run acceptance — safety boundaries', () => {
    ...
    it('parse-check sees through an aliasing obfuscation to a disallowed tool', async () => {
        ...
        const r = await run(`const t = tools; const sys = t.system; await sys.admin({});`, [safe]);
        expect(r.ok).toBe(false);
    ...
    it('the whitelist is the runtime boundary: a tool outside the allowed set is absent / throws', async () => {
        ...
        const r = await runToolOrchestration(`const name = ['secrets', 'exfiltrate'].join(':');
       try { await tools.call(name, {}); return { reached: true }; }
       catch (e) { return { reached: false, msg: String(e.message || e) }; }`, { ctx: MAKE_CTX(), deps: DEPS, tools: [a, b], allowed: new Set(['work_items:list']) });
        expect(r.ok).toBe(true);
        const s = r.summary;
