...
const mkTool = (name, effect, fn, effectForCall) => ({
    pluginName: 'fix',
    description: name,
...
describe('runToolOrchestration (B-CX-2A — code:run core, real dispatcher)', () => {
    it('forwards frozen runtime inputs through the orchestration composition seam', async () => {
        const r = await runToolOrchestration(`return { value: inputs.query, frozen: Object.isFrozen(inputs) };`, {
            ctx: MAKE_CTX(),
            deps: DEPS,
            tools: [],
    ...
    it('runs a multi-step script through the REAL dispatcher and returns only the summary', async () => {
        ...
        const get = mkTool('wi:get', 'read', async (a) => json({ id: a.id, ok: a.id !== 2 }));
       ...
       return { scanned: l.items.length, bad };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list, get] });
    ...
    describe('P-027 explicit final media result', () => {
        it('extracts validated image/audio blocks while preserving the authored summary', async () => {
            const r = await runToolOrchestration(`return {
        ...
        it('does not implicitly emit media returned by an intermediate tool call', async () => {
            ...
            const r = await runToolOrchestration(`await tools.assets.preview({}); return { inspected: true };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [preview] });
            expect(r.ok).toBe(true);
            expect(r.summary).toEqual({ inspected: true });
            expect(r.media).toBeUndefined();
        });
        it('does not reinterpret a legacy summary object without a media field', async () => {
            const r = await runToolOrchestration(`return { summary: { still: 'nested' }, count: 1 };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [] });
            expect(r.ok).toBe(true);
    ...
    describe('P-017 functions.exec helper parity', () => {
        it('streams notify/yield_control and emits a validated generated image with hint metadata', async () => {
            const emit = vi.fn();
            const r = await runToolOrchestration(`notify({ phase: 'render' });
         yield_control();
         generatedImage({ image_url: 'data:image/png;base64,aGVsbG8=', output_hint: 'preview' });
         return 'rendered';`, { ctx: MAKE_CTX({ emit }), deps: DEPS, tools: [] });
        ...
        it('rejects a generated image that is not a base64 image data URL', async () => {
            const r = await runToolOrchestration(`generatedImage({ image_url: 'https://example.invalid/image.png' }); return 'bad';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [] });
            expect(r.ok).toBe(false);
            expect(r.media).toBeUndefined();
        ...
        it('replays store/load state only when the prior result is explicitly rebound', async () => {
            const first = await runToolOrchestration(`store('cursor', load('cursor') + 1); return 'first';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [], inputs: { cursor: 1 } });
            const replayed = await runToolOrchestration(`store('cursor', load('cursor') + 1); return load('cursor');`, { ctx: MAKE_CTX(), deps: DEPS, tools: [], inputs: first.state });
    ...
    it('dryRun RECORDS write-effect calls without executing them; reads still run', async () => {
       ...
       return { processed: l.items.length };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list, setStatus], dryRun: true });
        expect(r.ok).toBe(true);
    ...
    it('resolves argument-sensitive effects before applying the dry-run gate', async () => {
        ...
        const deploy = mkTool('release:deploy', 'write', deployFn, (args) => (args.op === 'status' ? 'read' : 'write'));
        const r = await runToolOrchestration(`const status = await tools.release.deploy({ op: 'status' });
       await tools.release.deploy({ op: 'trigger' });
       return { status };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [deploy], dryRun: true });
        expect(r.ok).toBe(true);
    ...
    it('without dryRun, write-effect calls execute (and are still recorded for audit)', async () => {
        const writeFn = vi.fn(async () => json({ ok: true }));
        const setStatus = mkTool('wi:set-status', 'write', writeFn);
        const r = await runToolOrchestration(`await tools.wi.setStatus({ id: 1 }); return 'done';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [setStatus] });
        expect(r.ok).toBe(true);
    ...
    it('a write-effect call that resolves with ok:false (without throwing) is tallied in okFalseMutations', async () => {
        const setState = mkTool('wi:set-state', 'write', async (a) => json({ ok: false, error: 'completion_integrity_required', id: a.id }));
        const r = await runToolOrchestration(`const results = [];
       ...
       return { closedCount: results.length };`, // the script never checks .ok — mirrors the reported bug
        ...
        expect(r.summary.closedCount).toBe(2); // the script's own (wrong) count
    ...
    it('a read call returning ok:false makes an otherwise successful script partial', async () => {
        ...
        const r = await runToolOrchestration(`await tools.dev.pgQuery({ sql: 'bad' }); return { inspected: true };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [read] });
        expect(r.ok).toBe(true);
        expect(r.partial).toBe(true);
        expect(r.childFailures).toEqual([
    ...
    it('a write-effect call returning a bulk envelope with a per-item failure is tallied in okFalseMutations, even though the envelope\'s own top-level ok is true', async () => {
        const addItem = mkTool('plans:add-item', 'write', async () => json({ ok: true, results: [{ ok: false, slug: 'missing-plan', error: 'not_found' }], counts: { ok: 0, failed: 1 } }));
        const r = await runToolOrchestration(`const res = await tools.plans.addItem({ slug: 'missing-plan', phase: 'Phase 1', text: 't', importance: 'normal' });
       return { addItemOk: res.ok };`, // the script's own (misleadingly-true) top-level ok check
        { ctx: MAKE_CTX(), deps: DEPS, tools: [addItem] });
        expect(r.ok).toBe(true);
        expect(r.summary.addItemOk).toBe(true); // the script's own wrong read
        const okFalseMutations = r.okFalseMutations ?? [];
    ...
    it('a bulk envelope with zero per-item failures is NOT tallied as a semantic failure', async () => {
        const addItem = mkTool('plans:add-item', 'write', async () => json({ ok: true, results: [{ ok: true, slug: 's', itemId: 'P-001' }], counts: { ok: 1, failed: 0 } }));
        const r = await runToolOrchestration(`await tools.plans.addItem({ slug: 's', phase: 'Phase 1', text: 't', importance: 'normal' }); return 'done';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [addItem] });
        expect(r.okFalseMutations).toEqual([]);
    ...
    it('dryRun never populates okFalseMutations — write-effect calls are not executed', async () => {
        ...
        const r = await runToolOrchestration(`await tools.wi.setState({ id: 'A' }); return 'done';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [setState], dryRun: true });
        expect(r.okFalseMutations).toEqual([]);
    ...
    it('a good call still runs when a SIBLING ref is unknown — unknown rejects per-call (F8/H2)', async () => {
       ...
       return { items: l.items.length, badErr };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list] });
        expect(r.ok).toBe(true);
        expect(listFn).toHaveBeenCalledOnce(); // the good call executed (not nuked before running)
        expect(r.summary.items).toBe(1);
        expect(r.summary.badErr).toContain('not available in this sandbox');
    ...
    it('an unknown ref in an UNREACHED branch is a no-op — the run succeeds (F8)', async () => {
       ...
       return { n: l.items.length };`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list] });
        expect(r.ok).toBe(true);
    ...
    it('an UNCAUGHT unknown ref still fails the run, with the advisory to fix it (F8)', async () => {
        ...
        const r = await runToolOrchestration(`await tools.wi.list({}); await tools.system.admin({}); return 'done';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list] });
        expect(r.ok).toBe(false);
        expect(r.unknownRefs).toContain('system.admin');
...
describe('runToolOrchestration (B-CX-DEPS — host deps threaded into inner calls)', () => {
    it('records EACH inner tool call via deps.recordInvocation (inner calls are visible to spend/telemetry)', async () => {
        ...
        const list = mkTool('wi:list', 'read', async () => json({ items }));
        const get = mkTool('wi:get', 'read', async (a) => json({ id: a.id }));
        const r = await runToolOrchestration(`const l = await tools.wi.list({});
       for (const w of l.items) await tools.wi.get({ id: w.id });
       return { scanned: l.items.length };`, { ctx: MAKE_CTX({ runId: 'run_DEPS' }), deps, tools: [list, get] });
        expect(r.ok).toBe(true);
    ...
    it('an envelope-DENIED tool throws inside the script (and its handler never runs)', async () => {
        ...
        const deps = {
            checkCapabilityEnvelope: ({ toolName }) => toolName === 'wi:danger'
        ...
        const r = await runToolOrchestration(`await tools.wi.list({});
       try { await tools.wi.danger({}); return { threw: false }; }
       catch (e) { return { threw: true, msg: String(e && e.message || e) }; }`, { ctx: MAKE_CTX(), deps, tools: [allowed, denied] });
        expect(r.ok).toBe(true);
...
describe('runToolOrchestration (WI-1411 — wrapDispatch, per-inner-call context rebinding)', () => {
    it('wrapDispatch is invoked for EVERY inner call, with that call\'s own tool/name/args, and its next(callCtx) result is what the script sees', async () => {
       ...
       return got;`, {
            ...
            wrapDispatch: async (tool, toolName, args, ctx, next) => {
                ...
                return next({ ...ctx, workspaceId: `${ctx.workspaceId}-rebound` });
            },
        });
        expect(r.ok).toBe(true);
    ...
    it('derives distinct stable idempotency keys for keyed nested dispatches', async () => {
      ...
      return 'ok';
    `;
        const run = () => runToolOrchestration(script, {
...
describe('runToolOrchestration → dispatchCount (P-012)', () => {
    it('counts every dispatched call — reads and writes alike', async () => {
        ...
        const r = await runToolOrchestration(`const l = await tools.wi.list({});
       for (const w of l.items) await tools.wi.get({ id: w.id });
       return l.items.length;`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list, get] });
        expect(r.ok).toBe(true);
    ...
    it('a dryRun-skipped write is NOT counted (it never dispatched), but reads still are', async () => {
       ...
       return 'ok';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [list, setStatus], dryRun: true });
        expect(r.ok).toBe(true);
    ...
    it('a throwing dispatched call still counts (its row exists, status notwithstanding)', async () => {
        ...
        const r = await runToolOrchestration(`try { await tools.wi.boom({}); } catch {} return 'survived';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [boom] });
        expect(r.ok).toBe(true);
        expect(r.dispatchCount).toBe(1);
    });
    it('an unknown-ref stub rejection is NOT counted — the stub throws before the dispatcher', async () => {
        const r = await runToolOrchestration(`try { await tools.nope.thing({}); } catch {} return 'survived';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [] });
        expect(r.ok).toBe(true);
        expect(r.dispatchCount).toBe(0);
    });
});
describe('runToolOrchestration → runtime-owned safety observations (P-030)', () => {
    const principal = (capabilities) => ({
        kind: 'pi',
        slug: 'pi:caller',
        workspaceId: 'default',
    ...
    it('records every real-dispatch authorization entry outside the authored summary', async () => {
        ...
        const r = await runToolOrchestration(`await tools.wi.get({ id: 1 });
    ...
    it('detects an inner authority widening even when the script authors a clean verdict', async () => {
        ...
        const r = await runToolOrchestration(`await tools.wi.get({ id: 1 }); return { authorityUnchanged: true };`, {
            ctx: outer,
            deps: DEPS,
            tools: [read],
    ...
    it('does not count a dry-run-skipped write as an authorization observation', async () => {
        ...
        const r = await runToolOrchestration(`await tools.wi.setStatus({ id: 1 }); return 'preview';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [write], dryRun: true });
...
describe('runToolOrchestration → callRecords (P-013)', () => {
    it('records only secret-free call identity, disposition, and validated output references', async () => {
        const referenced = mkTool('assets:render', 'read', async () => json({
            ok: true,
        ...
        const r = await runToolOrchestration(`await tools.assets.render({ apiKey: 'never-emit-me' }); return 'ok';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [referenced] });
        expect(r.callRecords).toEqual([
            {
                ordinal: 0,
                tool: 'assets:render',
    ...
    it('preserves actual dispatch order and truthful dry-run/semantic/throw dispositions', async () => {
        ...
        const dry = await runToolOrchestration(`await tools.wi.list({}); await tools.wi.setStatus({ id: 'WI-1' }); return 'preview';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [read, reject], dryRun: true });
        ...
        const live = await runToolOrchestration(`await tools.wi.setStatus({ id: 'WI-1' }); try { await tools.wi.boom({}); } catch {} return 'done';`, { ctx: MAKE_CTX(), deps: DEPS, tools: [reject, boom] });
