...
const mkRoleTool = (name, agentRoles) => ({ expose: { mcp: { name } }, ...(agentRoles ? { agentRoles } : {}) });
describe('buildToolFacade (B-CX-1A)', () => {
    ...
    it('snake aliases respect the allowed-set whitelist (no bypass)', () => {
        const f = buildToolFacade([mkTool('work_items:list'), mkTool('system:admin')], vi.fn(), new Set(['work_items:list']));
...
describe('buildToolFacade: plugin-namespaced (dot-form) tool names (EI-18683272396981279)', () => {
    ...
    it('respects the allowed-set whitelist for dot-form names (no bypass)', () => {
        const f = buildToolFacade([mkTool('gitnexus.query'), mkTool('firecrawl.scrape')], vi.fn(), new Set(['gitnexus.query']));
