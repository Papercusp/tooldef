...
import { z } from 'zod';
import { applyEntityRefEnums, clearEntityEnums, clearEntityResolvers, collectEntityRefs, ENTITY_ENUM_CACHE_TTL_MS, entityEnumRevision, entityRef, entityRefMeta, formatEntityRefViolations, hasEntityResolver, resolveEntityRefs, setEntityEnum, setEntityResolver, } from './entity-ref';
...
describe('entityRef marker', () => {
    ...
    it('carries describe/max through to the published schema', () => {
        const js = z.toJSONSchema(z.object({ pot: entityRef('pot', { describe: 'the pot', max: 120 }) }));
...
describe('collectEntityRefs', () => {
    ...
    it('sees through optional/nullable/default wrappers', () => {
        const sites = collectEntityRefs(z.object({
    ...
    it('finds refs nested in objects and arrays', () => {
        const sites = collectEntityRefs(z.object({ members: z.array(z.object({ pot: entityRef('pot') })) }));
    ...
    it('finds refs inside a discriminated union, tagged by branch', () => {
        const sites = collectEntityRefs(z.discriminatedUnion('op', [
...
describe('resolveEntityRefs', () => {
    const potsResolver = (known) => vi.fn(async (values) => ({
...
describe('formatEntityRefViolations', () => {
    it('leads with the nearest match when there is one', async () => {
        ...
        const schema = z.object({ pot: entityRef('pot') });
        const msg = formatEntityRefViolations(await resolveEntityRefs(collectEntityRefs(schema), { pot: 'papercsp' }));
    ...
    it('names the allowed set when it is small enough to help', async () => {
        ...
        const schema = z.object({ pot: entityRef('pot') });
        const msg = formatEntityRefViolations(await resolveEntityRefs(collectEntityRefs(schema), { pot: 'zz' }));
    ...
    it('uses the custom label in the message', async () => {
        ...
        const msg = formatEntityRefViolations(await resolveEntityRefs(collectEntityRefs(schema), { p: 'zz' }));
...
describe('applyEntityRefEnums', () => {
    ...
    it('publishes NOTHING when the vocabulary exceeds the cap', async () => {
        ...
        const schema = z.object({ pot: entityRef('pot') });
        const out = await applyEntityRefEnums(schema, json(schema));
        expect(out.properties.pot.enum).toBeUndefined();
    });
    it('pairs the enum with a pattern for an open vocabulary, dropping the scalar type', async () => {
        ...
        const out = await applyEntityRefEnums(schema, json(schema));
        const node = out.properties.role;
    ...
    it('reaches a ref nested in an array of objects', async () => {
        ...
        const schema = z.object({ members: z.array(z.object({ pot: entityRef('pot') })) });
        const out = await applyEntityRefEnums(schema, json(schema));
        const members = out.properties.members;
        const item = members.items;
    ...
    it('single-flights one kind across a concurrent tools/list catalog walk', async () => {
        ...
        for (const out of results) {
            expect(out.properties.pot.enum).toEqual(['a']);
        }
    });
    it('reuses a successful fragment within the TTL and refreshes after it expires', async () => {
        ...
        expect(load).toHaveBeenCalledTimes(1);
        expect(first.properties.pot.enum).toEqual(['a']);
        expect(cached.properties.pot.enum).toEqual(['a']);
        now += ENTITY_ENUM_CACHE_TTL_MS + 1;
        const refreshed = await applyEntityRefEnums(schema, json(schema));
        expect(load).toHaveBeenCalledTimes(2);
        expect(refreshed.properties.pot.enum).toEqual(['b']);
    });
    it('does not cache a failed load and invalidates when a kind is re-registered', async () => {
        ...
        expect(failed.properties.pot.enum).toBeUndefined();
        expect(recovered.properties.pot.enum).toEqual(['a']);
        ...
        expect(replaced.properties.pot.enum).toEqual(['b']);
    });
...
describe('discriminated-union args', () => {
    ...
    it('checks the value once, not once per arm that declares the arg', async () => {
        const fn = vi.fn(async (values) => ({ unknown: values }));
    ...
    it('publishes the enum into every arm that declares the arg', async () => {
        ...
        const out = await applyEntityRefEnums(opUnion, toJsonSchema(opUnion));
        const arms = (out.anyOf ?? out.oneOf);
