...
const roleAllowlistStep = {
    name: 'role-allowlist',
    async run(exec) {
        const { tool, ctx, toolName } = exec;
        if (!tool.agentRoles || !ctx.role || ctx.gateBypass?.role)
            return null;
        if (tool.agentRoles.includes(ctx.role))
            return null;
        return {
            ok: false,
            error: {
 ...
 */
const roleRequirementStep = {
    name: 'role-requirement',
    async run(exec) {
        const { tool, ctx, toolName, deps } = exec;
        const required = tool.requireRoles;
        if (!required || required.length === 0 || ctx.gateBypass?.role)
            return null;
        const have = ctx.principal?.roles;
        if (have && required.some((r) => have.has(r)))
            return null;
        deps.auditAuth?.({
            ts: Date.now(),
...
const harnessCheckStep = {
    ...
    async run(exec) {
        const { tool, ctx, toolName } = exec;
        if (tool.harness !== 'required' || ctx.gateBypass?.harness)
            return null;
        ...
        return {
            ...
            error: {
                ...
                message: `Tool "${toolName}" requires a harness. Pass a harness slug (e.g. via ` +
    ...
    async run(exec) {
        ...
        if (replayCap > 0 && ctx.workspaceId && ctx.runId) {
            exec.bufferWriter = openReplayBuffer({
                ...
                onEvict: (ev) => {
                    // eslint-disable-next-line no-console
                    console.warn(`[replay-buffer] ${toolName}/${ctx.runId}: evicted event id=${ev.id} name=${ev.name} (capacity ${replayCap})`);
                },
            });
        }
        return null;
        ...
        if (tool.state && ctx.workspaceId && ctx.runId) {
            ...
            publishState = (snapshot) => {
                if (Array.isArray(snapshot)) {
                    throw new Error('ctx.publishState v1 is snapshot-only; pass a full state object, not a JSON-Patch array');
                }
    ...
    async run(exec) {
        ...
        try {
            ...
            if (exec.abort.signal.aborted) {
                ...
                // content.
                const handlerReportedFailure = result.isError === true ||
...
function extractInvalidInputErrorMetadata(error) {
    ...
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata))
        return undefined;
    const row = metadata;
    if (row.source !== PROJECTED_TOOL_REGISTRY_SOURCE ||
 ...
 */
function extractPostgresErrorMetadata(error) {
    ...
    const constraintName = typeof rawConstraint === 'string' && rawConstraint.length > 0
        ? rawConstraint.slice(0, POSTGRES_METADATA_STRING_MAX)
        : undefined;
    if (!sqlState && !constraintName)
        return undefined;
    return {
        postgres: {
            ...(sqlState ? { sqlState } : {}),
...
const authorizeStep = {
    ...
    async run(exec) {
        ...
        if (!authorize)
            return null; // no resource gate on this tool
...
function extractRefusalDetails(r) {
    ...
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            const fields = parsed;
            const code = [fields.error, fields.reason, fields.code, fields.errorCode].find((value) => typeof value === 'string' && value.length > 0);
...
function finalizeMetadata(exec) {
...
function mergeDispatchErrorMetadata(metadataJson, result) {
    ...
    if (!hasPostgres && !hasInvalidInput)
        return metadataJson;
    return {
        ...(metadataJson ?? {}),
