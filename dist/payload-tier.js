...
const GENERIC_PROJECTION_TARGET_CHARS = {
    trimmed: 7_000,
    standard: 18_000,
};
...
/** Test seam. */
export function resetPayloadTierRatchet() {
    ratchetWarned.clear();
}
...
function projectIdentityPreview(value, path, depth, state) {
    ...
    if (typeof value === 'bigint')
        return takePrimitive(state, value.toString(), path);
    if (typeof value === 'undefined')
        return takePrimitive(state, '[undefined]', path);
 ...
 */
function projectCursorArgs(args) {
    const source = args && typeof args === 'object' && !Array.isArray(args)
        ? args
        : {};
    const exact = { ...source, payloadTier: 'full' };
 ...
 */
function buildDefaultRecoveryNext(opts, argsTruncated) {
    const rawCall = opts.rawDispatchTemplate?.replaceAll('{tool}', opts.toolName);
    const lessHint = ` Prefer LESS over more: re-call with the tool's own narrowing args (limit/since/filters) ` +
        `or projection { pick: ["[].field"] } instead of paging a full payload.`;
    if (argsTruncated) {
        return (`Call ${opts.toolName} again with the ORIGINAL request args and payloadTier:'full' for full detail. ` +
    ...
    return (`Call ${opts.toolName} again with _projection.cursor.args for full detail. ` +
...
export function projectBoundedPayload(data, opts) {
    ...
    const metadata = {
        ...
        cursor: opts.recovery?.cursor ?? {
            ...
            ...(cursorArgs.truncated ? { argsTruncated: true } : {}),
        },
    ...
    if (metadata.returnedChars >= Math.min(target, PAYLOAD_TIER_HARD_CEILING_CHARS)) {
        ...
        const identityFields = identityOnly && typeof identityOnly === 'object' && !Array.isArray(identityOnly)
            ? identityOnly
            : null;
        const sourceFields = data && typeof data === 'object' && !Array.isArray(data)
            ? data
            : null;
        if (identityFields && sourceFields) {
            for (const key of opts.preserveTopLevelKeys ?? []) {
                if (!Object.prototype.hasOwnProperty.call(sourceFields, key) || key in identityFields)
                    continue;
...
export function applyPayloadTier(opts) {
    ...
    if (tierShaper) {
        try {
            ...
            // handle it).
            out = { ...response, data: tierShaper(response.data, { args, tier: tier }) };
