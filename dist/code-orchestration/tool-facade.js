...
export function splitToolName(name) {
    ...
    if (ci > 0)
        return { rawNs: name.slice(0, ci), rawVerb: name.slice(ci + 1) };
    const di = name.indexOf('.');
    if (di > 0)
        return { rawNs: name.slice(0, di), rawVerb: name.slice(di + 1) };
    return null;
}
/**
...
unknownRefs) {
    ...
    for (const tool of tools) {
        ...
        if (!split)
            continue; // skip names without a `ns:verb` / `ns.verb` shape
 ...
 * nest code-mode). A tool with no `agentRoles` is role-open; an empty list is an explicit deny-all
 * allowlist. This MIRRORS the dispatcher's role-allowlist gate (`dispatch-stack.ts` / `listMcpProjections`)
