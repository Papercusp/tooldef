 ...
 */
export type PrincipalKind = 'harness' | 'system' | 'pi' | 'user' | 'device' | 'service' | 'loopback';
/**
 ...
 */
export type PrincipalAuthMethod = 'bearer-token' | 'cookie-session' | 'jwt' | 'spawn-url' | 'host-loopback' | 'process-internal';
/**
...
export interface RouteDefinition<TInputSchema extends ZodTypeAny | undefined = undefined> {
    ...
    handler: (req: Request, ctx: RouteContext<TInputSchema extends ZodTypeAny ? z.infer<TInputSchema> : undefined>) => Response | Promise<Response>;
}
/**
 ...
 */
export interface RoleToolDefinition<TArgs extends StandardSchemaV1 = StandardSchemaV1, TEvents extends EventsSchema = EventsSchema> {
    name: string;
    /** Resource-authorization hook (RFC tooldef-auth Phase 1b) — see `Authorizer`. */
    authorize?: Authorizer<StandardSchemaV1.InferOutput<TArgs>, UnifiedToolContext, UnifiedToolContext['principal']>;
    /** RBAC role requirement (RFC tooldef-auth Phase 2): caller's `principal.roles` must include one of these (any-of). See `ProjectedTool.requireRoles`. */
...
export interface PromptDefinition {
    ...
    render: (args: Record<string, string>, ctx: PromptContext) => Promise<PromptResult>;
}
 ...
 */
export interface OpenCardSnapshot extends Omit<WireOpenCardSnapshot, 'presentation' | 'dataSchemaJson'> {
    presentation?: CardPresentation;
