...
export function registerPrompt(def) {
    ...
    if (def.name.startsWith(SLASH_PROMPT_PREFIX)) {
        throw new Error(`Prompt name "${def.name}" uses the reserved "${SLASH_PROMPT_PREFIX}" namespace ` +
...
export function lookupPrompt(name) {
...
export function _resetPromptCatalogForTests() {
    CATALOG.clear();
}
