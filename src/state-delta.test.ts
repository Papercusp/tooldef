...
describe('diffSnapshot / applySnapshotDelta', () => {
  ...
  it('emits only the cards that changed (add / update), not the unchanged ones', () => {
    ...
    expect(d.upsertedCards.map((c) => c.correlationId).sort()).toEqual(['b', 'c']); // 'a' unchanged → omitted
    expect(d.removedCardIds).toEqual([]);
  ...
  it('records removed cards', () => {
    ...
    const d = diffSnapshot(prev, next);
    expect(d.removedCardIds).toEqual(['b']);
    expect(d.upsertedCards).toEqual([]);
  ...
  it('is insensitive to object-key ordering (no false "updated")', () => {
    ...
    expect(diffSnapshot(prev, next).upsertedCards).toEqual([]);
  });
});
