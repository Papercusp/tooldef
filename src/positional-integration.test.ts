...
describe("positional write shim — end-to-end (P-008/P-009)", () => {
  it("advertises a single `row` string for a registry write-positional tool", () => {
    ...
    const listing = listMcpProjections().find(
      (l) => l.name === "wi:set_state",
    )!;
    const advertised = advertisedArgsSchema(
      "wi:set_state",
      listing.inputSchema as Record<string, unknown>,
    );
    const props =
      (advertised as { properties?: Record<string, unknown> }).properties ?? {};
    expect(Object.keys(props)).toEqual(["row"]);
    expect((props.row as { description?: string }).description).toContain(
      "id, state, harness?",
    );
  });
  ...
  it("reconstructs a positional `{ row }` into typed args before the handler", async () => {
    ...
    configurePrePromptRegistry([
      { name: "wi:set_state2", write: "positional" },
    ]);
  ...
  it("GUARD: a misaligned row (bad enum) is rejected, not silently written", async () => {
    ...
    configurePrePromptRegistry([
      { name: "wi:set_state3", write: "positional" },
    ]);
    const r = await dispatchProjectedTool(
  ...
  it("tolerates the natural `WI-12, passed` spacing end-to-end (trim normalization)", async () => {
    ...
    configurePrePromptRegistry([
      { name: "wi:set_state_ws", write: "positional" },
    ]);
    const r = await dispatchProjectedTool(
  ...
  it("keyed args still work for a non-prompt-aware caller", async () => {
    ...
    configurePrePromptRegistry([
      { name: "wi:set_state4", write: "positional" },
    ]);
    const r = await dispatchProjectedTool(
  ...
  it("bulk-capable positional tools advertise row+keyed oneOf and dispatch both shapes", async () => {
    ...
    expect(
      (
        (oneOf![0] as { properties?: Record<string, unknown> }).properties
          ?.row as { description?: string }
      ).description,
    ).toContain("id, state, harness?");
    expect(
      (oneOf![1] as { properties?: Record<string, unknown> }).properties,
    ).toHaveProperty("ids");
  ...
  it("a non-registry tool keeps its keyed args schema", () => {
    ...
    const listing = listMcpProjections().find(
      (l) => l.name === "wi:set_state5",
    )!;
    const props =
      (listing.inputSchema as { properties?: Record<string, unknown> })
        .properties ?? {};
  ...
  it("trailing free-text keeps embedded commas through the full path", async () => {
    ...
    expect(received).toEqual({
      id: "WI-9",
      body: "fixed it, added a test, shipped",
    });
  });
});
...
describe("anti-desync: legend columns === wire columns (P-011)", () => {
  it("the prompt legend, the read serializer, and the write shim share one projection", async () => {
    ...
    defineTool({
      ...
      args: z.object({}),
      result: z.array(
        z.object({ id: z.string(), state: z.enum(["todo", "done"]) }),
      ),
    ...
    const legend = renderWireSchemas(listPrePromptEntries(), (e) =>
      e.name === "wi:rlist"
        ...
        : {
            write: projectWriteColumns(
              wTool.inputSchema as Record<string, unknown>,
            ),
          },
    );
    ...
    expect((rr.result!.content[0] as { text: string }).text).toBe(
      "format: csv\n[1]\nWI-1,todo",
    );

    // WRITE: the shim reconstructs by the SAME column order, end-to-end.
    const writeCols = projectWriteColumns(
      wTool.inputSchema as Record<string, unknown>,
    )!;
    expect(writeCols.map((c) => c.name)).toEqual(["id", "state"]);
    expect(reconstructArgs("WI-9,done", writeCols)).toEqual({
      ok: true,
      args: { id: "WI-9", state: "done" },
    });
    const r = await dispatchProjectedTool(
      wTool,
      "wi:wstate",
...
describe("robustness — no collateral damage + graceful failure", () => {
  ...
  it("Tier-3 SAFE FALLBACK: a flat-schema read tool whose RUNTIME row has a nested field → TOON, not broken CSV", async () => {
    defineTool({
      ...
      handler: async () => ({
        data: [{ id: "a", state: "todo", extra: { nested: 1 } }] as never,
      }),
    });
    configurePrePromptRegistry([{ name: "rb:badshape", read: "csv" }]);
    const r = await dispatchProjectedTool(
      lookupByMcpName("rb:badshape")!,
      "rb:badshape",
      {},
      ctx(),
      DEPS,
  ...
  it("shim edge inputs: empty row rejected; non-string row falls through to Zod (no crash)", async () => {
    ...
    const empty = await dispatchProjectedTool(
      lookupByMcpName("rb:edge")!,
      "rb:edge",
      { row: "" },
      ctx(),
      DEPS,
    ...
    const nonString = await dispatchProjectedTool(
      lookupByMcpName("rb:edge")!,
      "rb:edge",
      { row: 123 },
      ctx(),
  ...
  it("advertisedArgsSchema is idempotent (double application stays {row})", () => {
    ...
    const once = advertisedArgsSchema(
      "rb:idem",
      listing.inputSchema as Record<string, unknown>,
    );
    ...
    expect(
      Object.keys(
        (twice as { properties?: Record<string, unknown> }).properties ?? {},
      ),
    ).toEqual(["row"]);
  });
});
...
describe("needsWorkspaceTx projection parity (EI-18808330244321407)", () => {
  it.each([
    ["principal", {}],
    ["role", { requirePrincipal: false as const }],
  ])(
    "a %s-gated defineTool preserves the explicit tx opt-in",
    (_kind, gate) => {
