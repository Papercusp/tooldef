...
const shape = {
  trimmed: (d: unknown) => ({ rows: (d as ReturnType<typeof rows>).rows.slice(0, 2), tierMark: SHAPED_MARK }),
  standard: (d: unknown) => ({ rows: (d as ReturnType<typeof rows>).rows.slice(0, 6), tierMark: SHAPED_MARK }),
};
...
for (const wrapper of ['principal-gated (legacy)', 'role-gated'] as const) {
  const define = (name: string, ignoreSessionPayloadTier?: boolean) =>
    defineTool({
      name,
      capability: 'test:read',
      description: 'fixture',
      ...(wrapper === 'role-gated' ? { requirePrincipal: false as const } : {}),
      args: z.object({}),
      shape,
  ...
  const call = async (name: string, over: Partial<UnifiedToolContext> = {}, args: Record<string, unknown> = {}) =>
    readBack(await dispatchProjectedTool(lookupByMcpName(name)!, name, args, ctx(over), DEPS));
